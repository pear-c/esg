...
function fnSearch() {
    ...
    if (fnPreSearch())
    {
        ...
        gfnTransation("/Promotion/Search",data,"POST",fnSearchCallback)
    }
...
function fnSearchDetailCallback(data) {
    ...
    for(var i=1;i< 6;i++){
    	...
    	if(resultMap['ATTACH_FILE_URL'+i] != null){
    		...
    		fileView += '<a href="/Promotion/FileDown?rid='+resultMap.RID+'&num='+i+'" title="다운로드" style="color: blue;">'+fileName+'</a>'
    	}
    }
...
function fnInsert() {
	if (fnPreInsert()){
			...
			if(fileflag == 1){
				fileUpload('/Promotion/FileUpload',$("#writerUserId").val(),gParamData);
			}else if(bannerflag == 1){
				fileUpload2("/Promotion/FileUpload",$("#writerUserId").val(),gParamData,$("#fileUpload2"),'bannerImgPath')
			}else{
				gfnSetProcessBar('on')
				fnFileUrlNullUpdate();
				...
				gfnTransation("/Promotion/Save",gParamData,"POST",fnUpdateCallback);
...
function fnUpdate() {
	...
	gfnTransation("/Promotion/SearchDetail",data,"POST",fnUpdateView)
}
...
function fnDelete() {
    ...
    gfnTransation("/Promotion/Delete",paramData,"POST",fnDeleteCallback)
}
...
function fnFileUrlNullUpdate(){
	...
	gfnTransation("/Promotion/FileDelete",data,"POST",null)
...
function fileUpload(url,id, inputParam){
    ...
    var sendUrl = url+"?type=promotion&Id="+id;
...
function fileUpload2(url,id, inputParam,data,param){
    ...
    var sendUrl = url+"?type=promotionBanner&Id="+id;
...
function fnInitComp() {
    ...
    var combo = [
        ...
        {id: "writeGubun", upprCode: "PROMOTION_CATEGORY", isAll: false},
        {id: "searchGubun", upprCode: "PROMOTION_CATEGORY", isAll: true}
	...
	$("#noticeWrite").click(function(){
		...
		$("input[name='toRcptYmd']").val(gfnGetDate(0));
		$("#noticeUploadFileList div").remove();
		//namoEditor.editorTarget.SetBodyValue('');
		quill.setContents();
...
function fnNoticeView(rid){
	...
	gfnTransation("/Promotion/SearchDetail",data,"POST",fnSearchDetailCallback)

	//gfnTransation("/Promotion/SearchSub",data,"POST",fnSearchSubCallback)
}
