...
function fnSearch() {
    ...
    if (fnPreSearch())
    {
        ...
        gfnTransation("/ErrorReport/Search",data,"POST",fnSearchCallback)
    }
...
function fnSearch2() {
    ...
    if (fnPreSearch())
    {
        ...
        gfnTransation("/ErrorReport/SearchReply",data,"POST",fnSearchReplyCallback)
    }
...
function fnSearchReplyCallback(data){
        ...
        for(var i=0; i <resultMap.length;i++){
            ...
            try{
	            if(resultMap[i].REPLY_ID != null){
	            	...
	            	str += '	<span class="name"'; if(replyIdData[1] == 'R999' || resultMap[i].USER_ROLE == 'RDEV') str += 'style="color : blue"';  str += '>' + replyIdData[0] + '</span>'
    ...
    $(".replayUpdate").click(function(){
       ...
       .after('<input type="hidden" class="replyOrigin" value="'+origin+'">')
        $("#replyUpdate"+replayId).click(function(){
            ...
            gfnTransation("/ErrorReport/UpdateReply",data,"POST",null)
    ...
    $(".replayDelete").click(function(){
        ...
        var data = new Object();
        data.id = replayId;
        gfnTransation("/ErrorReport/DeleteReply",data,"POST",null)
        fnSearch2()
    ...
    $(".replayAnswer").click(function(){
       ...
       $(this).parent().prevAll('textarea.comment_text').val('')
        ...
        $("#replyAnswer"+replayId).click(function(){
            ...
            data.contents = changeContents;
            gfnTransation("/ErrorReport/SaveReply",data,"POST",fnSearch2)
...
function fnSearchDetailCallback(data) {
   ...
   //$("#NamoSE_Ifr__editor2").contents().find('#NamoSE_editorpreview_editor2').contents().find('body').html(resultMap.CONTENTS)

   //namoEditor2.editorTarget.SetBodyValue(resultMap.CONTENTS);

    contentsPopData = resultMap.CONTENTS;
    ...
    for(var i=1;i< 6;i++){
    	...
    	if(resultMap['ATTACH_FILE_URL'+i] != null){
    		...
    		fileView += '<a href="/ErrorReport/FileDown?rid='+resultMap.RID+'&num='+i+'" title="다운로드" style="color: blue;">'+fileName+'</a>'
    	}
    }
...
function fnInsert() {
	if (fnPreInsert()){
			...
			if(fileflag == 1){
				fileUpload('/ErrorReport/FileUpload',$("#writerUserId").val(),gParamData);
			}else if(bannerflag == 1){
				fileUpload2("/ErrorReport/FileUpload",$("#writerUserId").val(),gParamData,$("#fileUpload2"),'bannerImgPath')
			}else{
				...
				gfnTransation("/ErrorReport/Save",gParamData,"POST",fnUpdateCallback);
...
function fnInsert2() {
    ...
    gfnTransation("/ErrorReport/SaveReply",data,"POST",fnInsertCallback);
...
function fnUpdate() {
	...
	gfnTransation("/ErrorReport/SearchDetail",data,"POST",fnUpdateView)
}
...
function fnDelete() {
    ...
    gfnTransation("/ErrorReport/Delete",paramData,"POST",fnDeleteCallback)
}
...
function fnFileUrlNullUpdate(){
	...
	gfnTransation("/ErrorReport/FileDelete",data,"POST",null)
...
function fileUpload(url,id, inputParam){
	 var sendUrl = url+"?type=errorReport&Id="+id;
...
function fnInitComp() {
    ...
    var combo = [
        ...
        {id: "writeGubun", upprCode: "ERRORREPORT_CATEGORY", isAll: false},
        {id: "searchGubun", upprCode: "ERRORREPORT_CATEGORY", isAll: true}
...
function fnNoticeView(rid,id){
	...
	gfnTransation("/ErrorReport/SearchDetail",data,"POST",fnSearchDetailCallback)
	...
	//gfnTransation("/ErrorReport/SearchSub",data,"POST",fnSearchSubCallback)
}
